...
3. Provides a small server runtime (`<SayScope>` and `getSay()`) for server-rendered apps.
...
They are fragments rather than whole messages, so they nest inside a `<Say>`:
...
That form has only the one locale the server sent, so it cannot switch: `set` rejects anything else. Switch by navigating, and let the server render the new locale. See [`<SayScope>`](#sayscope), which fills these props in for you.
...
```ts
import { SayScope, getSay } from '@saykit/react/server';
```

The two halves mirror each other: `<SayScope>` is to `getSay()` what `<SayProvider>` is to `useSay()`. Each establishes a view for its subtree, and each hands the same kind of value back.

### `<SayScope>`

`<SayScope>` establishes the view for everything rendered inside it. It negotiates the locale against your catalogue, loads its messages, and publishes the view into React's per-request `cache()`, so a concurrent request rendering another locale reads its own.
...
`<SayProvider>` takes no props here. In a server component it resolves to the `react-server` build of `@saykit/react/client`, which reads the enclosing scope and serialises that locale and its messages across the boundary, which is all a client component can be given.

If you have already resolved the view yourself, hand it over instead of the catalogue and locale:
...
<Callout type="warn">
  One scope per request. A scope does not end where its children do: React renders a server
  component's children after it returns, so there is nowhere to put the previous view back. A second
  `<SayScope>` establishing another locale in the same request takes over for everything rendered
  after it, including components outside it and the messages `<SayProvider>` serialises to the
  client. Development warns when it happens. To render a second locale, give it its own request, or
  resolve its view yourself with `catalogue.load(locale)` and pass it to the components that need
  it.
</Callout>

### `getSay`

`getSay()` returns the view the enclosing `<SayScope>` established. It is the server counterpart of `useSay()`:
...
It throws if no `<SayScope>` is above the component calling it. The view is per request, so a concurrent request rendering another locale gets its own.
...
- on the server, `<SayScope>` binds one view per request, and `getSay()` reads it
