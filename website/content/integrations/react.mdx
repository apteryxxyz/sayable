...
<Callout type="info">
  A line break between two children is layout, not a space, exactly as it is in JSX itself, so the
  `{' '}` above is what keeps "due" from running into the date. It extracts as a plain space inside
  the message, and Prettier writes it for you when it wraps a line that ends in one. See
  [whitespace](/core-concepts/messages#whitespace).
</Callout>
