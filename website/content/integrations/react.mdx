...
const say = catalogue.locale(catalogue.match([navigator.language]));
...
Internally, `SayProvider` builds a `View` over the locale and messages you pass in. Anything beneath it, `<Say>`, `<Say.Plural>`, `useSay()`, resolves against that locale.
...
`useSay()` returns the `View` for the provider's locale. It is immutable, so it formats and reads messages but cannot switch locale; that is the provider's job.
...
setSay(catalogue.locale('fr'));
```

A view is already immutable, so `setSay()` stores exactly what you hand it. The registration itself is request-scoped, and nothing about the view can change under a concurrent request.

### `getSay`

`getSay()` returns the registered view for the current request. It's the server equivalent of `useSay()`:
...
3. takes that locale's view off the shared catalogue
