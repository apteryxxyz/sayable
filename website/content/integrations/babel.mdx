...
The import is replaced with the record, in the module that imported it. Babel alone is enough, with no bundler configuration at all.
...
<Callout type="warn">
  ...
  not. For dynamic locale loading, write a thunk per locale in `messages` and let your bundler
  handle the imports.
</Callout>
