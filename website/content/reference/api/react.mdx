...
<TypeTable
  type={{
    ...
    locale: {
      description: 'The locale to bind. Use with `messages`, instead of `store`.',
      type: 'string',
    },
    messages: {
      description:
        'The compiled messages for that locale. Should be referentially stable rather than a fresh object literal per render.',
      type: 'View.Messages',
    },
    children: {
...
There is no hook for the store behind it. A store is a module-scope value you already hold, so switching is `store.set('fr')` on the one you built.
