...
<TypeTable
  type={{
    ...
    'one, two, few, many, zero, other': {
      description: 'CLDR plural-category branches. `other` is required.',
      type: 'string',
    },
    '_1, _2, …': {
      description:
...
<TypeTable
  type={{
    Component: {
      description: 'A server component to wrap. Receives `locale` and `messages` props plus the originals.',
