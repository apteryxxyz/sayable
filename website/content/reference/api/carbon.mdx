...
<TypeTable
  type={{
    catalogue: {
      description:
        'The shared `Catalogue` to register. Should be the same one you bind `createWithSay` to.',
      type: 'Catalogue',
      required: true,
    },
  }}
...
When the base class extends `BaseCommand` (`Command`, `CommandWithSubcommands`, &hellip;), the resulting constructor accepts a mapping function:
