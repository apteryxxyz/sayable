...
The first guess that satisfies either step wins. If no guess matches, `defaultLocale` is returned as a fallback, which is the first configured locale unless you set one.
...
- **Re-render**: hold `catalogue.locale(newLocale)` in state and re-render the tree.
