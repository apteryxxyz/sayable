...
<Callout type="warn">
  Versions before `@saykit/config@0.3.0` emitted `{locale}.{extension}.d.ts`. That name happened to work for `.po` (TypeScript falls back to appending `.d.ts` for extensions it doesn't know) but was ignored for `.json`, so JSON catalogues got the literal type inferred from the file's current contents. Delete any leftover `*.*.d.ts` files after upgrading.
</Callout>
