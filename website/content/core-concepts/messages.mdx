...
<Callout type="info">
  Branch keys that start with a digit get a leading underscore in JSX (`_1`, `_2`) because JSX
  prop names can't start with a number. SayKit strips the underscore at extract time.
</Callout>
