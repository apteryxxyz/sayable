...
`{` and `}` are ICU's own syntax, so a message that means them literally has to say so. JSX has no
way to write a bare brace in text at all, which leaves the expression form, and extraction escapes
it for you on the way into the catalogue:

```tsx
<Say>Use {'{'}name{'}'} here</Say>
// Use '{'name'}' here   →   renders as: Use {name} here
```

The same applies to a `say` template, where a brace can be written directly:
...
An apostrophe is the character ICU escapes with, so it is doubled where it sits in front of a brace
or another apostrophe, and left alone everywhere else — `It's` stays `It's`. A `#` is never
escaped, since inside a plural that is the number being formatted.
