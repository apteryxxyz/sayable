...
- translation fallback chains, configured with `fallbackLocales` in your config

Today, the `Say` instance itself does not walk a per-message fallback chain at runtime. Instead, fallback translations are applied during extraction, based on [`fallbackLocales`](/core-concepts/configuration).
