...
Every locale needs an entry in `messages`, written either way round:
...
A thunk is called once, and two loads that overlap share that one call rather than fetching twice. A thunk that rejects is forgotten, so the locale can be asked for again.
