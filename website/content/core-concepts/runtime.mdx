...
store.current; // the view for 'en'
store.locale; // 'en'

const unsubscribe = store.subscribe((say) => render(say));

await store.set('fr');
store.current; // the view for 'fr'
```

`set()` loads the locale first if the catalogue does not have it yet, and like `load()` it is synchronous when your loader is, so a locale that is already loaded switches inside the same tick.
...
// `store.current` at the point you format, not once into a module binding.
