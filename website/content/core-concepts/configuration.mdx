...
The file is handed to the runtime as it sits on disk, with nothing copied, transpiled or cached, so
...
Extraction only sees messages that appear as macros in source files. When a string belongs in the catalogue but has no call site, such as a browser extension manifest, an app store listing, an email subject or database seed data, declare it on the bucket instead of writing a dummy module for the extractor to find.
