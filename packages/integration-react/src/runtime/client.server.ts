 ...
 * cross is the locale and its messages, which are compiled to data rather than
 * to functions for exactly this reason, so this reads them off the view the
 * enclosing segment established with {@link import('./server.js').setSay} and
 * passes them to the real provider, which is why `<SayProvider>` written on
 * the server takes no props.
 */
